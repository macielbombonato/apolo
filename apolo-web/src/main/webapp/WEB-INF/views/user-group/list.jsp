										...
										<tbody>
											<c:forEach items="${userGroupList}" var="group">
												<tr id="userGroup_${group.id}">
													...
													<td>
														<s:message code="user.status.${group.status}" />
													</td>
													<security:authorize  ifAnyGranted="ROLE_ADMIN, ROLE_USER_PERMISSION_VIEW, ROLE_USER_PERMISSION_EDIT, ROLE_USER_PERMISSION_REMOVE">
														<td>
															<div class="btn-group btn-group-sm">
																<security:authorize  ifAnyGranted="ROLE_ADMIN, ROLE_USER_PERMISSION_VIEW">
																	<a href='<s:url value="/${currentUser.tenant.url}/user-group/view"></s:url>/${group.id}' class="btn btn-small btn-default" data-toggle="tooltip" title="<s:message code="common.show" />">
																		<i class="glyphicon glyphicon-zoom-in"></i>
																	</a>
																</security:authorize>
																<security:authorize  ifAnyGranted="ROLE_ADMIN, ROLE_USER_PERMISSION_EDIT">
																	<a href='<s:url value="/${currentUser.tenant.url}/user-group/edit"></s:url>/${group.id}' class="btn btn-small btn-default" data-toggle="tooltip" title="<s:message code="common.edit" />">
																		<i class="glyphicon glyphicon-edit"></i>
																	</a>
																</security:authorize>
																<security:authorize  ifAnyGranted="ROLE_ADMIN, ROLE_USER_PERMISSION_REMOVE">
																	<a href='#' class="btn btn-small btn-default" onclick="removeConfirmationDialogOpen('<s:url value="/${currentUser.tenant.url}/user-group/remove"></s:url>/${group.id}', 'userGroup_${group.id}');" data-toggle="tooltip" title="<s:message code="common.remove" />">
