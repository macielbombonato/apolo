...
<input type="hidden" id="tenant.id" name="tenant.id" value="${userGroup.tenant.id}" />
<input type="hidden" id="tenant.name" name="tenant.name" value="${userGroup.tenant.name}" />
<input type="hidden" id="tenant.url" name="tenant.url" value="${userGroup.tenant.url}" />
		...
		<c:choose>
			<c:when test="${not readOnly}">
				<div class="form-group">
					...
					<select name="permissions" id="listTo" size="5" multiple="multiple" class="form-control chzn-select" <c:if test="${readOnly}">disabled="disabled"</c:if> data-placeholder='<s:message code="common.select" />'>
